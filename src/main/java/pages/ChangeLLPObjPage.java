...
public class ChangeLLPObjPage extends BasePage {

    private final WaitUtils wait;
    SafeActionUtils safe;

    public ChangeLLPObjPage(WebDriver driver) {
        super(driver);
        this.wait = new WaitUtils(driver, 30);
        this.safe = new SafeActionUtils(driver);
    }
    @FindBy(xpath = "//input[@placeholder='Enter LLP Name']")
    private WebElement inputLLPName;

    @FindBy(xpath = "//input[@placeholder='Enter LLPIN']")
    private WebElement inputLLPIN;

    @FindBy(xpath = "//input[@placeholder='Enter Objects to be added']")
    private WebElement inputObjectsToBeAdded;

    @FindBy(xpath = "(//input[contains(@id,'react-select') and @role='combobox'])[1]")
    private WebElement replaceObject;

    @FindBy(xpath = "(//input[contains(@id,'react-select') and @role='combobox'])[2]")
    private WebElement addedNewObject;
    ...
    public WebElement LLPINbutton;

    @FindBy(xpath = "//button[@class='font-medium text-[14px] rounded-[4px] flex items-center gap-[5px] p-[7px] px-[10px] relative overflow-hidden bg-[#022B50] border-[1px] border-[#022B50] text-white w-full justify-center event-track cursor-pointer hover:bg-opacity-90']")
    public WebElement Objectivesbutton;

    @FindBy(xpath = "//button[@class='font-medium text-[14px] rounded-[4px] flex items-center gap-[5px] p-[7px] px-[10px] relative overflow-hidden bg-[#022B50] border-[1px] border-[#022B50] text-white w-full justify-center py-[1.2rem] event-track cursor-pointer hover:bg-opacity-90']")
    public WebElement lastbutton;

    @FindBy(xpath = "(//input[contains(@id,'react-select') and @role='combobox'])[1]")
    private WebElement annualFilings;

    @FindBy(xpath = "(//input[contains(@id,'react-select') and @role='combobox'])[2]")
    private WebElement stampPaper;

    public void typeAnnualFilings(String value) {
        safe.safeType(annualFilings, value, "Annual Filings");
        annualFilings.sendKeys(Keys.ENTER);
    }

    public void typeStampPaper(String value) {
        safe.safeType(stampPaper, value, "Stamp Paper");
        stampPaper.sendKeys(Keys.ENTER);
    }

    public void typeLLPName(String llpName) {
        safe.safeType(inputLLPName, llpName, "Enter LLP Name");
    }

    public void typeLLPIn(String llpin) {
        safe.safeType(inputLLPIN, llpin, "Enter LLP In");
    }

    public void typeObjectsToBeAdded(String objects) {
        safe.safeType(inputObjectsToBeAdded, objects, "Enter Objects To Be Added");
    }

    public void typeReplaceObject(String value) {
        safe.safeType(replaceObject, value, "Replace Existing Object");
        replaceObject.sendKeys(Keys.ENTER);
    }

    public void typeAddedNewObject(String value) {
        safe.safeType(addedNewObject, value, "Add New Object");
        addedNewObject.sendKeys(Keys.ENTER);
    }
    ...
    public void LLPINbutton() {
        safe.safeClick(LLPINbutton, "Next Button");
    }

    public void Objectivesbutton() {
        safe.safeClick(Objectivesbutton, "Next Button");
    }

    public void lastbutton() {
        safe.safeClick(lastbutton, "Next Button");
    }

    public void selectingtheservice5() {
        safe.safeClick(selectingtheservice5, "Selecting these services");
    }

    @FindBy(xpath = "//li[contains(@class, 'select2-results__option') and contains(text(), 'Change the Objectives of Your LLP')]")
